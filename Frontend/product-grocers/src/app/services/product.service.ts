...
export class ProductService {
  ...
  getProducts(): Observable<Products[]> {
    ...
    return this.http.get<Products[]>("http://3.80.137.241:9080/product/allProductDetails");
  }

  
  //post method 1st parameter url and 2nd parameter json data. 
  storeProductDetailsInfo(productRef: any) {
    this.http.post("http://3.80.137.241:9080/product/storeProductDetails", productRef, { responseType: "text" }).
      subscribe(result => console.log(result), error => console.log(error));
  }
  retrieveAllProductDetails(): Observable<Product[]> {
    //this.pro = this.http.get<Product[]>("http://3.80.137.241:9080/product/allProductDetails");
    return this.http.get<Product[]>("http://3.80.137.241:9080/product/allProductDetails")
  }

  retrieveProductById(id: any): Observable<Product[]> {
    return this.http.get<Product[]>("http://3.80.137.241:9080/product/retrieveProductById/" + id)
  }

  //by default all HttpClient method return type is observable with json format data. 
  deleteProductById(id: any): any {
    return this.http.delete("http://3.80.137.241:9080/product/deleteProductById/" + id, { responseType: 'text' });
  }

  updateProduct(productRef: any): any {
    return this.http.put("http://3.80.137.241:9080/product/updateProduct", productRef, { responseType: 'text' })
  }

  reduceQuantity(productRef: any): any {
    return this.http.put("http://3.80.137.241:9080/product/reduceQuantity", productRef, { responseType: 'text' })
  }
